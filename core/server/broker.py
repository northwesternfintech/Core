...
class Broker:
    ...
    def __init__(self,
        ...
        self._mgr_be = self._context.socket(zmq.ROUTER)
        self._mgr_be.bind(manager_address)
    ...
    async def _start_manager(self):
        ...
        mgr_startup_poller.register(self._mgr_be, zmq.POLLIN)
        ...
        for _ in range(5):
            ...
            if socks.get(self._mgr_be) == zmq.POLLIN:
                frames = self._mgr_be.recv_multipart()
    ...
    async def _manage_client_fe(self):
        ...
        while True:
            ...
            await self._mgr_be.send_multipart(frames)

    async def _manage_mgr_be(self):
        ...
        while True:
            socks = await self._mgr_be_poller.poll(self._heartbeat_timeout_s * 1000)
            socks = dict(socks)

            if socks.get(self._mgr_be_poller) == zmq.POLLIN:
                frames = await self._hearbeat_socket.recv_multipart()
            ...
            if time.time() >= heartbeat_at:
                ...
                self._hearbeat_socket.send_multipart(msg)
    ...
    async def _manage_worker_be(self):
        ...
        1. Periodically send heartbeats to workers
        2. Check for heartbeats from workers. If a worker is unresponsive, send a
        ...
        while not self._kill.is_set():
            ...
            if socks.get(self._backend_socket) == zmq.POLLIN:
                ...
                if len(msg) == 1:
                    if msg[0] == protocol.READY:
                        ...
                        self._worker_last_seen[address] = time.time()
                    elif msg[0] == protocol.DIE:
                        death_message = msg[1]
                        msg = [self._manager_uuid, protocol.DIE, address, death_message]
            ...
            for worker in self._workers:
                ...
                if self._worker_liveness[worker] == 0:
                    ...
                    msg = [self._manager_uuid, protocol.DIE, address, b"timeout"]
                    await self._worker_be.send_multipart(msg)
